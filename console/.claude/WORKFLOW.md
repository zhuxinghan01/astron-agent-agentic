...
**流程**：
```
Issue 分析 → 定位代码 → 修复 → 验证 → /doc-module（如涉及业务逻辑变更）
```

**说明**：
- 不需要走 skills 链路
- 如果修复涉及业务逻辑变更，需要更新 `module.md`
...
### 阶段 5: 技术设计 → `/backend-design` + `/frontend-design`
...
**输出**:
- `console/docsForAi/{feature-name}/backend-design.md`
...
**输出**: 更新 `console/docsForAi/{module-name}/module.md`
...
**执行**:
1. 将 `console/docsForAi/{feature-name}/` 下的文档归档到 Git
2. 在 `console/docsForAi/overview.md` 中添加该功能的索引
3. 删除临时文件（如有）
...
2. **定位代码**
   ...
   - 读取 `console/docsForAi/{module}/module.md` 快速了解模块
...
**则使用新功能开发流程**:
1. 创建 `console/docsForAi/bugfix-{issue-number}/`
...
**快速链路** (简单功能):
```
/requirement → /spec → /tasks → 实现 → /doc-module
```
...
# 1. 需求分析
/requirement
# 输出: console/docsForAi/bot-tag-management/requirement.md

# 2. 用户故事
/stories
# 输出: console/docsForAi/bot-tag-management/stories.md

# 3. 技术规格
/spec
# 输出: console/docsForAi/bot-tag-management/spec.md

# 4. 任务拆解
/tasks
# 输出: console/docsForAi/bot-tag-management/tasks.md
...
# 7. 更新模块文档
/doc-module
# 输出: 更新 console/docsForAi/bot-management/module.md
...
- [docsForAi Overview](../docsForAi/overview.md) - 模块文档索引
